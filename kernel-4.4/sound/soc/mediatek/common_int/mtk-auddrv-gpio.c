...
enum audio_system_gpio_type {
	...
	GPIO_EXTAMP_HIGH,
	GPIO_EXTAMP_LOW,
#ifdef CONFIG_LGE_HAC_AMP_SUPPORT
	GPIO_HACAMP_HIGH,
	GPIO_HACAMP_LOW,
#endif
	GPIO_NUM,
	GPIO_AUD_CLK_MOSI_OFF = GPIO_NUM,
...
#ifdef CONFIG_MACH_MT6739_MH6
static struct audio_gpio_attr aud_gpios[GPIO_NUM_MH6] = {
...
#ifdef CONFIG_LGE_HAC_AMP_SUPPORT
	[GPIO_HACAMP_HIGH] = {"hacamp-pullhigh", false, NULL},
	[GPIO_HACAMP_LOW] = {"hacamp-pulllow", false, NULL},
#endif
...
void AudDrv_GPIO_probe(void *dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(aud_gpios); i++) {
		...
		if (IS_ERR(aud_gpios[i].gpioctrl)) {
			...
			pr_info("%s() : [LGE_Audio] pinctrl_lookup_state %s ok", __func__, aud_gpios[i].name);
...
#ifdef CONFIG_LGE_HAC_AMP_SUPPORT
int AudDrv_GPIO_HAC_AMP_Select(int bEnable, int mode)
{
	int retval = 0;
#if MT6755_PIN
	int i = 0;
	...
	if (bEnable == 1) {
		if (aud_gpios[GPIO_HACAMP_HIGH].gpio_prepare) {
			for (i = 0; i < mode; i++) {
				...
				if (retval)
					pr_err("could not set aud_gpios[GPIO_HACAMP_LOW] pins\n");
				udelay(2);
				retval = pinctrl_select_state(pinctrlaud,
						aud_gpios[GPIO_HACAMP_HIGH].gpioctrl);
				if (retval)
					pr_err("could not set aud_gpios[GPIO_HACAMP_HIGH] pins\n");
				udelay(2);
			}
		}
	} else {
		if (aud_gpios[GPIO_HACAMP_LOW].gpio_prepare) {
			retval =
			    pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_HACAMP_LOW].gpioctrl);
			if (retval)
				pr_err("could not set aud_gpios[GPIO_HACAMP_LOW] pins\n");
		}
	}
