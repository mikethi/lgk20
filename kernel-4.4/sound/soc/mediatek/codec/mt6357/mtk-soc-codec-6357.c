...
static void Ext_HAC_Amp_Change(bool enable)
{
	pr_debug("%s(), enable %d\n", __func__, enable);
	if (enable) {
		AudDrv_GPIO_HAC_AMP_Select(false, 3);
		usleep_range(1 * 1000, 2 * 1000);
		AudDrv_GPIO_HAC_AMP_Select(true, 3);
		usleep_range(5 * 1000, 10 * 1000);
	} else {
		AudDrv_GPIO_HAC_AMP_Select(false, 3);
		udelay(500);
...
static int Ext_HAC_Amp_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] =
	    mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTHACAMP];
	return 0;
}
static int Ext_HAC_Amp_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	if (ucontrol->value.integer.value[0]) {
		Ext_HAC_Amp_Change(true);
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTHACAMP] =
		    ucontrol->value.integer.value[0];
	} else {
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTHACAMP] =
		    ucontrol->value.integer.value[0];
		Ext_HAC_Amp_Change(false);
	}
	return 0;
}
#endif
...
#ifdef CONFIG_LGE_HAC_AMP_SUPPORT
	SOC_ENUM_EXT("Ext_HAC_Amp_Switch", Audio_DL_Enum[15],
		     Ext_HAC_Amp_Get, Ext_HAC_Amp_Set),
#endif
};
