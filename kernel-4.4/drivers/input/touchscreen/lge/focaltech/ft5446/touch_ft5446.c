...
static void project_param_set(struct device *dev)
{
	...
	/* if model support knock on, lpwg swipe, set function on*/
	d->p_param.use_lpwg_function = FUNC_OFF;
}
...
static int ft5446_deep_sleep(struct device *dev, int mode)
{
	...
	if(mode) {
		...
		return ft5446_reg_write(dev, 0xA5, &data, 1);
	}
	else {
...
#if 0
static void ft5446_debug_tci(struct device *dev)
{
	struct ft5446_data *d = to_ft5446_data(dev);
	...
	ft5446_reg_read(dev, TCI_DEBUG_R, &rdata, sizeof(rdata));
	...
	if (d->state == TC_STATE_ACTIVE) {
		...
		if (ts->lpwg.screen == 0) {
			if (ts->lpwg.sensor == PROX_NEAR) {
				...
				next_state = TC_STATE_DEEP_SLEEP;
				TOUCH_I("STATE_ACTIVE to STATE_DEEP_SLEEP (LPWG_NONE)\n");
				...
				ret = ft5446_lpwg_control(dev, ts->lpwg.mode, true);
			}
		} else {
		...
		if (ts->lpwg.screen == 0) {
			if(ts->lpwg.mode == LPWG_NONE){
				...
				TOUCH_I("DEEP_SLEEP to DEEP_SLEEP (LPWG_NONE)\n");
...
#if 0
static void ft5446_lcd_event_read_reg(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	u32 rdata[5] = {0};

//	ft5446_xfer_msg_ready(dev, 5);
...
#if 0
static int ft5446_wireless_status(struct device *dev, u32 onoff)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int ft5446_earjack_status(struct device *dev, u32 onoff)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static void ft5446_fb_notify_work_func(struct work_struct *fb_notify_work)
{
	struct ft5446_data *d =
			container_of(to_delayed_work(fb_notify_work),
				struct ft5446_data, fb_notify_work);
	int ret = 0;
...
static int ft5446_notify(struct device *dev, ulong event, void *data)
{
	...
	case NOTIFY_CALL_STATE:
		...
		ret = ft5446_reg_write(dev, REG_CALL_STATE,
			(u32 *)data, sizeof(u32));
		break;
...
static int ft5446_probe(struct device *dev)
{
	...
	d->tci_debug_type = LPWG_DEBUG_ALWAYS;
	d->swipe_debug_type = LPWG_DEBUG_ALWAYS;
	atomic_set(&ts->state.debug_option_mask, DEBUG_OPTION_2);
...
#if 0 // JASON MH4+ FT5446
	// Reset CTPM
	TOUCH_I("%s - Reset CTPM\n", __func__);
	touch_msleep(50);

	i2c_buf[0] = 0xAA;
	ret = ft5446_reg_write(dev, 0xFC, i2c_buf, 1);
	if(ret < 0) {
	         TOUCH_E("reset CTPM strat error\n");
	         return -EIO;
	}
	mdelay(5);
...
static int ft5446_suspend(struct device *dev)
{
	...
	if(d->p_param.use_lpwg_function == FUNC_ON){
    	...
    	ft5446_power(dev,POWER_OFF);  //only for No-LPWG Model
	}
...
static int ft5446_resume(struct device *dev)
{
    ...
    if(d->p_param.use_lpwg_function == FUNC_ON){
        ...
        ft5446_power(dev,POWER_ON);  //only for No-LPWG Model
	}
...
int ft5446_debug_info(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft5446_data *d = to_ft5446_data(dev);
	struct ft5446_touch_debug *debug = &d->info.debug;
	int ret = 0;
	int i = 0;
...
int ft5446_report_swipe_fr_buffer(struct device *dev)
{
	...
	struct ft5446_data *d = to_ft5446_data(dev);
	int ret = 0, i;
	u8 swipe_fr_buffer[1 + SWIPE_FAILREASON_BUF_LEN], swipe_fr_cnt, swipe_fr;
	...
	if (swipe_fr_cnt > SWIPE_FAILREASON_BUF_LEN) {
		...
		for (i = 0; i < swipe_fr_cnt; i++) {
			swipe_fr = swipe_fr_buffer[1 + i];
			if (swipe_fr < SWIPE_FAILREASON_NUM) {
				TOUCH_I("Swipe Failure Reason Buffer [%02d] : [%s]\n", i+1, swipe_debug_str[swipe_fr]);
			}
			else {
				TOUCH_I("Swipe Failure Reason Buffer [%02d] : [%s]\n", i+1, swipe_debug_str[SWIPE_FAILREASON_NUM]);
			}
		}
	}
...
int ft5446_irq_handler(struct device *dev)
{
	...
	if(d->state == TC_STATE_POWER_OFF){
		TOUCH_I("%s called but touch ic power off. \n",__func__);
		return -EPERM;
	}
...
int ft5446_power(struct device *dev, int ctrl)
{
	...
	case POWER_OFF:
		...
		if(d->state == TC_STATE_ACTIVE){
			...
			TOUCH_I("%s, Already touch power off. skip \n", __func__);
	...
	case POWER_ON:
		...
		if(d->state == TC_STATE_POWER_OFF){
			...
			TOUCH_I("%s, Already touch power on. skip \n", __func__);
