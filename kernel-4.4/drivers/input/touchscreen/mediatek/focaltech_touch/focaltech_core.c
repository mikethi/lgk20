...
int fts_power_init(void)
{
    int ret;
    /*set TP volt*/
    printk("qiang power in statr\n");
    tpd->reg = regulator_get(tpd->tpd_dev, "VTOUCH");
    ret = regulator_set_voltage(tpd->reg, 2800000, 2800000);
    if (ret != 0) {
    	printk("qiang Failed to set voltage of regulator,ret=%d!", ret);
    ...
    if (ret != 0) {
        printk("qiang Fail to enable regulator when init,ret=%d!", ret);
...
static int tpd_local_init(void)
{
    FTS_FUNC_ENTER();
#if FTS_POWER_SOURCE_CUST_EN
	printk("qiang power start\n");
    if (fts_power_init() != 0)
        return -1;
	printk("qiang power end\n");
#endif
