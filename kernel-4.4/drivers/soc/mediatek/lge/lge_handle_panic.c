...
static int read_crash_handle_enable(char *buffer, struct kernel_param *kp)
{
...
static void lge_print_mtk_poweroff_reason(void * buffer)
{
	...
	if( local_crash_fp != NULL )
	{
		...
		pr_err("[pmic_status] POFFSTST=0x%x\n", local_crash_fp->pmic_poweroff_reason.POFFSTST);
		if ((local_crash_fp->pmic_poweroff_reason.POFFSTST >> 0) & 1)
			pr_err("[pmic_status] power off_reason is UVLO event\n");
		if ((local_crash_fp->pmic_poweroff_reason.POFFSTST >> 3) & 1)
			pr_err("[pmic_status] power off_reason is Thermal shutdown\n");
		if ((local_crash_fp->pmic_poweroff_reason.POFFSTST >> 4) & 1)
			pr_err("[pmic_status] power off_reason is WARM Reset\n");
		if ((local_crash_fp->pmic_poweroff_reason.POFFSTST >> 5) & 1)
			pr_err("[pmic_status] power off_reason is Cold Reset\n");
		if ((local_crash_fp->pmic_poweroff_reason.POFFSTST >> 6) & 1)
			pr_err("[pmic_status] power off_reason is Long press shutdown\n");
		if ((local_crash_fp->pmic_poweroff_reason.POFFSTST >> 7) & 1)
			pr_err("[pmic_status] power off_reason is PWRHOLD clear(NORMOFF)\n");
		if ((local_crash_fp->pmic_poweroff_reason.POFFSTST >> 10) & 1)
			pr_err("[pmic_status] power off_reason is AP Watchdog\n");

		pr_err("[pmic_status] PONSTST=0x%x\n", local_crash_fp->pmic_poweroff_reason.PONSTST);
		if ((local_crash_fp->pmic_poweroff_reason.PONSTST >> 0) & 1)
			pr_err("[pmic_status] power on_reason is PWRKEY press\n");
		if ((local_crash_fp->pmic_poweroff_reason.PONSTST >> 1) & 1)
			pr_err("[pmic_status] power on_reason is RTC alarm\n");
		if ((local_crash_fp->pmic_poweroff_reason.PONSTST >> 2) & 1)
			pr_err("[pmic_status] power on_reason is WARM charger insertion\n");
		if ((local_crash_fp->pmic_poweroff_reason.PONSTST >> 3) & 1)
			pr_err("[pmic_status] power on_reason is SPAR event\n");
		if ((local_crash_fp->pmic_poweroff_reason.PONSTST >> 4) & 1)
			pr_err("[pmic_status] power on_reason is Cold Reset\n");
#else
