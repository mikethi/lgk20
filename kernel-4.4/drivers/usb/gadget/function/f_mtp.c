				...
				const struct usb_ctrlrequest *ctrl)
{
	...
	if (ctrl->bRequestType ==
		...
		if (ctrl->bRequest == MTP_REQ_CANCEL && w_index == dev->mtp_id
				&& w_value == 0) {
...
mtp_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	dev->eps_in = krealloc(dev->eps_in, sizeof(*dev->eps_in) * (ep_id + 1), GFP_KERNEL);
	if (IS_ERR_OR_NULL(dev->eps_in))
		return -ENOMEM;
	dev->eps_in[ep_id] = dev->ep_in;

	dev->eps_out = krealloc(dev->eps_out, sizeof(*dev->eps_out) * (ep_id + 1), GFP_KERNEL);
	if (IS_ERR_OR_NULL(dev->eps_out))
		return -ENOMEM;
	dev->eps_out[ep_id] = dev->ep_out;

	dev->eps_intr = krealloc(dev->eps_intr, sizeof(*dev->eps_intr) * (ep_id + 1), GFP_KERNEL);
