...
#ifdef CONFIG_MTK_PMIC_CHIP_MT6356
static unsigned int step_isink0 = CONFIG_LGE_LEDS_DEFAULT_STEP_ISINK0;
module_param(step_isink0, uint, 0644);
#endif

static unsigned int step_isink1 = CONFIG_LGE_LEDS_DEFAULT_STEP_ISINK1;
module_param(step_isink1, uint, 0644);
#endif
...
int mt_led_blink_pmic(enum mt65xx_led_pmic pmic_type, struct nled_setting *led)
{
	...
	LEDS_DEBUG("LED blink on time = %d offtime = %d\n",
		   led->blink_on_time, led->blink_off_time);
#ifdef CONFIG_LGE_LEDS
	time_index = (led->blink_on_time + led->blink_off_time - 2) / 2;
	if (time_index <  0)
		time_index = 0;
	if (time_index > 65535)
		time_index = 65535;
...
#ifdef CONFIG_LGE_LEDS
		pmic_set_register_value(PMIC_ISINK_CH0_STEP, step_isink0);
...
#ifdef CONFIG_LGE_LEDS
		pmic_set_register_value(PMIC_ISINK_CH1_STEP, step_isink1);
...
#ifdef CONFIG_LGE_LEDS
		pmic_set_register_value(PMIC_ISINK_CH0_STEP, step_isink0);
		pmic_set_register_value(PMIC_ISINK_DIM0_DUTY, level);
#else /* MediaTek */
		pmic_set_register_value(PMIC_ISINK_CH0_STEP, ISINK_3);	/* 16mA */
...
#ifdef CONFIG_LGE_LEDS_TRIGGER_PATTERN
static struct nled_breath_setting patterns[] = {
	{LED_PATTERN_POWER_ON, 255, 7, 2, 7, 4, 3, 0},
	{LED_PATTERN_POWER_OFF, 255, 7, 2, 7, 4, 3, 0},
	{LED_PATTERN_CHARGING, 255, 7, 2, 7, 4, 3, 0},
	{LED_PATTERN_ALARM, 255, 0, 0, 0, 0, 0, 0},
	{LED_PATTERN_INCOMING_CALL, 255, 2, 0, 0, 0, 0, 0},
	{LED_PATTERN_URGENT_INCOMING_CALL, 255, 0, 0, 0, 0, 0, 0},
};

static struct nled_breath_setting pattern_default = {
	0, 255, 7, 2, 7, 4, 3, 0
};

static struct nled_breath_setting *find_breath_setting(unsigned long id) {
	int i;
...
static int mt_led_breath_pmic(enum mt65xx_led_pmic pmic_type, unsigned long id)
{
	struct nled_breath_setting *breath = NULL;

	LEDS_DEBUG("mt_led_breath_pmic: pmic_type=%d\n", pmic_type);
	LEDS_DEBUG("LED pattern id %lu\n", id);

	/* TODO */
	breath = find_breath_setting(id);

	LEDS_DEBUG("LED breath tr1=%u, tr2=%u, tf1=%u, tf2=%u, ton=%u, toff=%u\n",
			breath->tr1, breath->tr2, breath->tf1, breath->tf2,
			breath->ton, breath->toff);
	...
	case MT65XX_LED_PMIC_NLED_ISINK1:
		...
		pmic_set_register_value(PMIC_ISINK_CH1_STEP, ISINK_3);	/* 16mA */
#endif
		pmic_set_register_value(PMIC_ISINK_BREATH1_TR1_SEL, breath->tr1);
		pmic_set_register_value(PMIC_ISINK_BREATH1_TR2_SEL, breath->tr2);
		pmic_set_register_value(PMIC_ISINK_BREATH1_TF1_SEL, breath->tf1);
		pmic_set_register_value(PMIC_ISINK_BREATH1_TF2_SEL, breath->tf2);
		pmic_set_register_value(PMIC_ISINK_BREATH1_TON_SEL, breath->ton);
		pmic_set_register_value(PMIC_ISINK_BREATH1_TOFF_SEL, breath->toff);
		pmic_set_register_value(PMIC_ISINK_DIM1_DUTY, breath->duty);
		pmic_set_register_value(PMIC_ISINK_DIM1_FSEL, ISINK_128K_500HZ);	/* 1KHz */
		pmic_set_register_value(PMIC_ISINK_CH1_BIAS_EN, NLED_ON);
		pmic_set_register_value(PMIC_ISINK_CHOP1_EN, NLED_ON);
		pmic_set_register_value(PMIC_ISINK_CH1_EN, NLED_ON);
...
#ifdef CONFIG_LGE_LEDS_TRIGGER_PATTERN
int mt_mt65xx_pattern_set(struct led_classdev *led_cdev,
			  unsigned long *pattern_id)
{
	struct mt65xx_led_data *led_data = container_of(led_cdev,
			struct mt65xx_led_data, cdev);

	/* only support in pmic mde */
	if (led_data->cust.mode != MT65XX_LED_MODE_PMIC)
		return -1;

	if (*pattern_id == led_data->pattern_id)
		return 0;

	led_data->pattern_id = *pattern_id;

	if (led_data->pattern_id == LED_PATTERN_STOP) {
		mt_brightness_set_pmic(led_data->cust.data, 0, 0);
		return 0;
	}

	mt_led_breath_pmic(led_data->cust.data, led_data->pattern_id);
