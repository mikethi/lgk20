...
static int mt65xx_pattern_set(struct led_classdev *led_cdev,
			      unsigned long *pattern_id)
{
	if (mt_mt65xx_pattern_set(led_cdev, pattern_id))
		return -1;
	else
		return 0;
}
...
#ifdef CONFIG_LGE_LEDS_TRIGGER_PATTERN
		if (i == MT65XX_LED_TYPE_RED || i == MT65XX_LED_TYPE_GREEN
				|| i == MT65XX_LED_TYPE_BLUE) {
			g_leds_data[i]->cdev.pattern_set = mt65xx_pattern_set;
			g_leds_data[i]->level = -1;	/* not initialized */
		}
#endif
