...
#ifdef CONFIG_LGE_PM
	/* wait USB Ready in normal boot only */
...
static unsigned int hw_bc11_stepA1(void)
{
	 ...
	 *  So we check DP & DM are over 1.2V to check Apple TA or not.
	 */
...
#elif defined(CONFIG_LGE_PM_APPLE_CHARGER_PULLDOWN)
	/*
	 * Apple Charger Detection
	 * - for legacy apple charger (rounded square)
	 *  0.5A : DP 1.5V ~ 2.3V , DM < 2.3V
	...
	return wChargerAvailDP | wChargerAvailDM;
#else
	/* do not detect Apple TA if feature is not enabled */
	return 0;
#endif
#else /* MediaTek */
	unsigned int wChargerAvail = 0;
