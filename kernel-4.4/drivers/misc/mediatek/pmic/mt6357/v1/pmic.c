...
#ifdef CONFIG_LGE_LEDS
void pmic_leds_init(void)
{
	/* Turn off CHRIND */
	pmic_set_register_value(PMIC_CHRIND_EN_SEL, 0x1);
	pmic_set_register_value(PMIC_CHRIND_CHOP_SW, 0x0);
	pmic_set_register_value(PMIC_CHRIND_BIAS_EN, 0x0);
	pmic_set_register_value(PMIC_CHRIND_EN, 0x0);
...
void pmic_leds_shutdown(void)
{
	/* Enable : AUTO */
	pmic_set_register_value(PMIC_CHRIND_EN_SEL, 0x0);
}
#endif
