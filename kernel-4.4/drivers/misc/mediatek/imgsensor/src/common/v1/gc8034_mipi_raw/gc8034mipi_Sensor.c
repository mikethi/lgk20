...
unsigned char gc8034_otp_data[GC8034_OTP_SIZE] = {0};
unsigned char gc8034_module_info_is_valid = 0;
unsigned char gc8034_awb_is_valid = 0;
unsigned char gc8034_lsc_is_valid = 0;
unsigned char gc8034_af_is_valid = 0;
unsigned char gc8034_otp_is_valid = 0;
#define MODULE_INFO_START 0x00
#define MODULE_INFO_CHECKSUM 0x08
#define AWB_INFO_START 0x09
#define AWB_INFO_CHECKSUM 0x1A
#define AF_INFO_START 0x1B
#define AF_INFO_CHECKSUM 0x21
#define LSC_INFO_START 0x22
#define LSC_INFO_CHECKSUM 0x76F
#endif
...
#if GC8034_OTP
static u8 read_otp_cmos_sensor(kal_uint32 addr)
{
	u8 get_byte = 0;
...
static int read_otp(void){
	int i2c_tmp = imgsensor.i2c_write_id;
	int i, sum = 0;
	imgsensor.i2c_write_id = 0xA0;
	for(i = 0; i < GC8034_OTP_SIZE; i++){
					gc8034_otp_data[i] = read_otp_cmos_sensor(i);
	}
  imgsensor.i2c_write_id = i2c_tmp;
	/*check module info*/
	for(i = MODULE_INFO_START; i < MODULE_INFO_CHECKSUM; i++){
					sum += gc8034_otp_data[i];
	}
	if(((sum % 255 + 1) == gc8034_otp_data[MODULE_INFO_CHECKSUM]) && (0x55 == gc8034_otp_data[MODULE_INFO_START])){
			gc8034_module_info_is_valid = 1;
			sum = 0;
	}else{
			sum = 0;
			pr_err("=== module info invalid !!! ===\n");
	}
	/*check awb info*/
	for(i = AWB_INFO_START; i < AWB_INFO_CHECKSUM; i++){
					sum += gc8034_otp_data[i];
	}
	if(((sum % 255 + 1) == gc8034_otp_data[AWB_INFO_CHECKSUM]) && (0x55 == gc8034_otp_data[AWB_INFO_START])){
			gc8034_awb_is_valid = 1;
			sum = 0;
	}else{ 
			sum = 0;
			pr_err("=== awb info invalid !!! ===\n");
	}
	/*check AF info*/
	for(i = AF_INFO_START; i < AF_INFO_CHECKSUM; i++){
					sum += gc8034_otp_data[i];
	}
	if(((sum % 255 + 1) == gc8034_otp_data[AF_INFO_CHECKSUM]) && (0x55 == gc8034_otp_data[AF_INFO_START])){
			gc8034_af_is_valid = 1;
			sum = 0;
	}else{
			sum = 0;
			pr_err("=== af info invalid !!! ===\n");
	}
	/*check lsc info*/
	for(i = LSC_INFO_START; i < LSC_INFO_CHECKSUM; i++){
					sum += gc8034_otp_data[i];
	}
	if(((sum % 255 + 1) == gc8034_otp_data[LSC_INFO_CHECKSUM]) && (0x55 == gc8034_otp_data[LSC_INFO_START])){
			gc8034_lsc_is_valid = 1;
			sum = 0;
	}else{
			sum = 0;
			pr_err("=== lsc info invalid !!! ===\n");
	}

	/*check all data*/
	for(i = 0; i < GC8034_OTP_SIZE - 1; i++){
					sum += gc8034_otp_data[i];
	}
	if(((sum % 255 + 1) == gc8034_otp_data[GC8034_OTP_SIZE - 1])){
			gc8034_otp_is_valid = 1;
			sum = 0;
	}else{
			sum = 0;
			pr_err("=== all data check invalid !!! ===\n");
	}

	if(!gc8034_lsc_is_valid || !gc8034_af_is_valid || !gc8034_awb_is_valid || !gc8034_module_info_is_valid || !gc8034_otp_is_valid)
					return 0;
	else
					return 1;
}
