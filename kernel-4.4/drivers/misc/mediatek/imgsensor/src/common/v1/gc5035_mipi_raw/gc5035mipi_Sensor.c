...
unsigned char gc5035_otp_data_info[MODULE_INFO_SIZE];
unsigned char gc5035_otp_data_awb[AWB_INFO_SIZE];
unsigned char gc5035_module_id = 0;
unsigned char gc5035_awb_valid = 0;
#endif
...
#if defined(GC5035_OTP_CUSTOMER)
unsigned int info_grop1_addr[6] = {0x1f18, 0x1f20, 0x1f28, 0x1f30, 0x1f38, 0x1f40};
unsigned int info_grop2_addr[6] = {0x1f48, 0x1f50, 0x1f58, 0x1f60, 0x1f68, 0x1f70};
static int gc5035_otp_read_module_info(void)
{
	...
	if(flag_module & 0x01){
			pr_err("module info group 2 is valid!!\n");
			for(i = 0; i < MODULE_INFO_SIZE; i++){
						gc5035_otp_data_info[i] = gc5035_otp_read_byte(info_grop2_addr[i]);
			}
	}else if(flag_module & 0x04){
			pr_err("module info group 1 is valid!!\n");
			for(i = 0; i < MODULE_INFO_SIZE; i++){
						gc5035_otp_data_info[i] = gc5035_otp_read_byte(info_grop1_addr[i]);
			}
	}else{
			pr_err("=== gc5035 read_otp module info group invalid!! ===\n");
	    return 0;
	}

	for (i = 0; i < MODULE_INFO_SIZE - 1; i++)
		check += gc5035_otp_data_info[i];

	if ((check % 255 + 1) == gc5035_otp_data_info[MODULE_INFO_SIZE - 1]){
		module_id = gc5035_otp_data_info[0];
		lens_id = gc5035_otp_data_info[1];
		year = gc5035_otp_data_info[2];
		month = gc5035_otp_data_info[3];
		day = gc5035_otp_data_info[4];

		LOG_INF("module_id = 0x%x\n", module_id);
		LOG_INF("lens_id = 0x%x\n", lens_id);
		LOG_INF("data = %d-%d-%d\n", year, month, day);
	} else{
		pr_err("=== gc5035 read_otp module info group checksum failed!! ===\n");
	  return 0;
	}
		gc5035_module_id = module_id;
		return 1;
}

unsigned int awb_grop1_addr[5] = {0x1f80, 0x1f88, 0x1f90, 0x1f98, 0x1fa0};
unsigned int awb_grop2_addr[5] = {0x1fa8, 0x1fb0, 0x1fb8, 0x1fc0, 0x1fc8};
static int gc5035_otp_read_wb_info(void)
{
	kal_uint8  flag_wb = 0;
	kal_uint8  i = 0;
	int check = 0;
	...
	if(flag_wb & 0x01){
			pr_err("module awb group 2 is valid!!\n");
			for(i = 0; i < AWB_INFO_SIZE; i++){
						gc5035_otp_data_awb[i] = gc5035_otp_read_byte(awb_grop2_addr[i]);
			}
	}else if(flag_wb & 0x04){
			pr_err("module awb group 1 is valid!!\n");
			for(i = 0; i < AWB_INFO_SIZE; i++){
						gc5035_otp_data_awb[i] = gc5035_otp_read_byte(awb_grop1_addr[i]);
			}
	}else{
			pr_err("=== gc5035 read_otp module awb group invalid!! ===\n");
	    return 0;
	}

	for(i = 0; i < AWB_INFO_SIZE - 1; i++)
		check += gc5035_otp_data_awb[i];

	if ((check % 255 + 1) == gc5035_otp_data_awb[AWB_INFO_SIZE - 1]){
			LOG_INF("module awb checksum valid!!\n");
	}else{
			pr_err("=== gc5035 read_otp module awb checksum invalid!! ===\n");
	    return 0;
	}
	gc5035_awb_valid = 1;
	return 1;
...
#if defined(GC5035_OTP_CUSTOMER)
static int gc5035_otp_read(void)
{
	int rc1, rc2;
	rc1 = gc5035_otp_read_module_info();
	rc2 = gc5035_otp_read_wb_info();
	if(!rc1 || !rc2)
		return 0;
	return 1;
}
...
static int gc5035_otp_function(void)
{
  int rc;
...
#if defined(GC5035_OTP_CUSTOMER)
				sensor_init();
				rc = gc5035_otp_function();
				if(!rc){
					*sensor_id = 0xFFFFFFFF;
					pr_err("=== read otp failed ===\n");
