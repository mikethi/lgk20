...
unsigned char hi846_lsc_data[HI846_LSC_DATA_SIZE];
#define HI846_AWB_DATA_SIZE    8
unsigned char Hi846_awb_data[HI846_AWB_DATA_SIZE] = {0};
#define HI846_AF_DATA_SIZE    5
unsigned char Hi846_af_data[HI846_AF_DATA_SIZE] = {0};
unsigned char Hi846_vendor_id;
...
static u8 read_cmos_sensor(kal_uint32 addr)
{
	u8 get_byte=0;
	...
	iReadRegI2C(pu_send_cmd, 2, &get_byte, 1, imgsensor.i2c_write_id);
...
static u8 read_cmos_sensor_byte(kal_uint32 addr){
    u8 get_byte = 0;
    char pu_send_cmd[2] = {(char)(addr >> 8), (char)(addr & 0xFF) };
    kdSetI2CSpeed(400);
    iReadRegI2C(pu_send_cmd, 2, &get_byte, 1, imgsensor.i2c_write_id);
...
void HI846OTPSetting(void)
{
  LOG_INF("%s enter\n",__func__);
	...
	HI846_OTP_write_cmos_sensor(0x0d04, 0x00);//Fsync(OTP busy)Output Enable 
...
kal_uint16 Hi846_Sensor_OTP_info(void)
{
    kal_uint16 ModuleVendorID = 0,
    	     PositionID = 0,
    	     LensID = 0,
    	     VCMID = 0,    
    	     Year = 0,
	...
	info_flag = HI846_Sensor_OTP_read(0x0201);
	LOG_INF("%s,info_flag = 0x%x\n",__func__,info_flag);
	
	switch (info_flag)
	{
		case 0x01:  //Group1
		ModuleVendorID = HI846_Sensor_OTP_read(0x0202);
		PositionID = HI846_Sensor_OTP_read(0x0203);
		LensID = HI846_Sensor_OTP_read(0x0204);
		VCMID = HI846_Sensor_OTP_read(0x0205);
		Year = HI846_Sensor_OTP_read(0x0206);
		Month = HI846_Sensor_OTP_read(0x0207);
		Day = HI846_Sensor_OTP_read(0x0208);		
		infocheck = HI846_Sensor_OTP_read(0x0209);
		break;
		case 0x13:
		ModuleVendorID = HI846_Sensor_OTP_read(0x020A);
		PositionID = HI846_Sensor_OTP_read(0x020B);
		LensID = HI846_Sensor_OTP_read(0x020C);
		VCMID = HI846_Sensor_OTP_read(0x020D);
		Year = HI846_Sensor_OTP_read(0x020E);
		Month = HI846_Sensor_OTP_read(0x020F);
		Day = HI846_Sensor_OTP_read(0x0210);		
		infocheck = HI846_Sensor_OTP_read(0x0211);
    break;
		case 0x37:
    ModuleVendorID = HI846_Sensor_OTP_read(0x0212);
		PositionID = HI846_Sensor_OTP_read(0x0213);
		LensID = HI846_Sensor_OTP_read(0x0214);
		VCMID = HI846_Sensor_OTP_read(0x0215);
		Year = HI846_Sensor_OTP_read(0x0216);
		Month = HI846_Sensor_OTP_read(0x0217);
		Day = HI846_Sensor_OTP_read(0x0218);		
		infocheck = HI846_Sensor_OTP_read(0x0219);
		break;
    default:
      LOG_INF("HI846_Sensor: info_flag error value: %d \n ",info_flag);
    break;
   }
    checksum = (ModuleVendorID + PositionID + LensID + VCMID+ Year + Month + Day) % 255 + 1;
    ...
    if (checksum == infocheck)
		...
		Hi846_vendor_id = ModuleVendorID;
    LOG_INF("ModuleVendorID = %d \n", ModuleVendorID);
    LOG_INF("PositionID = %d \n", PositionID);
    LOG_INF("LensID = %d \n", LensID);
    LOG_INF("VCMID = %d \n", VCMID);  
...
void HI846_Sensor_calc_wbdata(void)
{
    ...
    //uint16_t r_gain = 0x200, b_gain = 0x200, g_gain = 0x200;
    uint16_t wb_unit_r = 0, wb_unit_b = 0, wb_unit_gr = 0, wb_unit_gb = 0,
             wb_golden_r = 0, wb_golden_b = 0, wb_golden_gr = 0, wb_golden_gb = 0;       

    //uint16_t rg_golden_value = 0, bg_golden_value = 0,
	     //rg_value = 0, bg_value = 0;    

    LOG_INF("%s enter\n",__func__);
    wb_flag = HI846_Sensor_OTP_read(0x021A);
    pr_debug("wb_flag = 0x%x %s\n",wb_flag,__func__);
	
    switch (wb_flag)
    {
	case 0x01:  //Group1 valid
        	wb_unit_r  = HI846_Sensor_OTP_read(0x021B);
        	wb_unit_b  = HI846_Sensor_OTP_read(0x021C);
        	wb_unit_gr = HI846_Sensor_OTP_read(0x021D);
		wb_unit_gb = HI846_Sensor_OTP_read(0x021E);

		wb_golden_r  = HI846_Sensor_OTP_read(0x021F);
		wb_golden_b  = HI846_Sensor_OTP_read(0x0220);
		wb_golden_gr = HI846_Sensor_OTP_read(0x0221);
		wb_golden_gb = HI846_Sensor_OTP_read(0x0222);
		   
		wbcheck = HI846_Sensor_OTP_read(0x0223);
			
		for(i=0;i < 8;i++)
		{
		    addr = 0x021B + i;
		    LOG_INF("HI846_Sensor: group 1 addr = 0x%x  0x%x\n ",i+0x021B,HI846_Sensor_OTP_read(addr));
		}
		break;	
	case 0x13:  //Group2 valid
		wb_unit_r  = HI846_Sensor_OTP_read(0x0224);
		wb_unit_b  = HI846_Sensor_OTP_read(0x0225);
		wb_unit_gr = HI846_Sensor_OTP_read(0x0226);
		wb_unit_gb = HI846_Sensor_OTP_read(0x0227);

		wb_golden_r  = HI846_Sensor_OTP_read(0x0228);
		wb_golden_b  = HI846_Sensor_OTP_read(0x0229);
		wb_golden_gr = HI846_Sensor_OTP_read(0x022A);
		wb_golden_gb = HI846_Sensor_OTP_read(0x022B);
		   
		wbcheck = HI846_Sensor_OTP_read(0x022C);
			
		for(i=0;i < 8;i++)
		{
		    addr = 0x0224 + i;
		    LOG_INF("HI846_Sensor: group 1 addr = 0x%x  0x%x\n ",i+0x0224,HI846_Sensor_OTP_read(addr));
		}
		break;
	case 0x37:  //Group3 valid
		wb_unit_r  = HI846_Sensor_OTP_read(0x022D);
		wb_unit_b  = HI846_Sensor_OTP_read(0x022E);
		wb_unit_gr = HI846_Sensor_OTP_read(0x022F);
		wb_unit_gb = HI846_Sensor_OTP_read(0x0230);

		wb_golden_r  = HI846_Sensor_OTP_read(0x0231);
		wb_golden_b  = HI846_Sensor_OTP_read(0x0232);
		wb_golden_gr = HI846_Sensor_OTP_read(0x0233);
		wb_golden_gb = HI846_Sensor_OTP_read(0x0234);
		   
		wbcheck = HI846_Sensor_OTP_read(0x0235);
			
		for(i=0;i < 8;i++)
		{
		    addr = 0x022D + i;
		    LOG_INF("HI846_Sensor: group 1 addr = 0x%x  0x%x\n ",i+0x022D,HI846_Sensor_OTP_read(addr));
		}
		break;
	default:
	...
	checksum = (wb_unit_r + wb_unit_b + wb_unit_gr + wb_unit_gb + wb_golden_r + wb_golden_b + wb_golden_gr + wb_golden_gb) % 255 + 1;
	if (checksum == wbcheck)
	{
...
kal_uint16 Hi846_Sensor_OTP_read_lsc(void)
{
    kal_uint16 tt = 0;
    kal_uint16 loop = 0;
    ...
    unsigned int hi846_lsc_checksum_addr = 0;

    LOG_INF("%s enter\n",__func__);

    if (HI846_Sensor_OTP_read(0x0249) == 0x01) {
        hi846_lsc_addr = 0x024A;
        hi846_lsc_checksum_addr = 0x0996;
    } else if(HI846_Sensor_OTP_read(0x0249) == 0x13) {
        hi846_lsc_addr = 0x0997;
        hi846_lsc_checksum_addr = 0x10E3;
    } else if(HI846_Sensor_OTP_read(0x0249) == 0x37) {
        hi846_lsc_addr = 0x10E4;
        hi846_lsc_checksum_addr = 0x1830;
    } else
        return 0;
    LOG_INF("%s,hi846_lsc_addr = 0x%x\n",__func__,hi846_lsc_addr);

    for (loop = 0; loop < HI846_LSC_DATA_SIZE; loop++){
        tt = HI846_Sensor_OTP_read(hi846_lsc_addr);
        hi846_lsc_data[loop] = tt;
        hi846_lsc_addr++;
    }
    for(loop = 0; loop < HI846_LSC_DATA_SIZE; loop++) {
				pr_err("=== AAA LSC[%d] : %x AAA ===\n", loop, hi846_lsc_data[loop]);
        checksum += hi846_lsc_data[loop];
    }
    checksum_value = HI846_Sensor_OTP_read(hi846_lsc_checksum_addr);
...
kal_uint16 HI846_Sensor_OTP_read_awb(void)
{    
    kal_uint16 tt = 0;
    kal_uint16 loop = 0;
    kal_uint32 checksum = 0;
    ...
    unsigned int hi846_awb_checksum_addr = 0;

    LOG_INF("%s enter\n",__func__);

    if (HI846_Sensor_OTP_read(0x021A) == 0x01) {
        hi846_awb_addr = 0x021B;
        hi846_awb_checksum_addr = 0x0223;
    } else if(HI846_Sensor_OTP_read(0x021A) == 0x13) {
        hi846_awb_addr = 0x0224;
        hi846_awb_checksum_addr = 0x022C;
    } else if(HI846_Sensor_OTP_read(0x021A) == 0x37) {
        hi846_awb_addr = 0x022D;
        hi846_awb_checksum_addr = 0x0235;
    } else
        return 0;
    
    LOG_INF("%s,hi846_awb_addr = 0x%x\n",__func__,hi846_awb_addr);

    for (loop = 0; loop < HI846_AWB_DATA_SIZE; loop++){
    ...
    for(loop = 0; loop < HI846_AWB_DATA_SIZE; loop++)
    {
				pr_err("=== AAA AWB[%d] : %x AAA ===\n", loop, Hi846_awb_data[loop]);
			checksum += Hi846_awb_data[loop];
    }
		checksum_value = HI846_Sensor_OTP_read(hi846_awb_checksum_addr);

    LOG_INF("%s: Hi846_awb checksum_value = 0x%x checksum = 0x%x,0x%x\n",__func__, checksum_value, checksum,((checksum % 0xff) + 1));
    if(((checksum % 0xff) + 1) == checksum_value)
...
kal_uint16 HI846_Sensor_OTP_read_af(void)
{    
    kal_uint16 tt = 0;
    kal_uint16 loop = 0;
    kal_uint32 checksum = 0;
    kal_uint16 checksum_value = 0;
    unsigned int hi846_af_addr = 0;
    unsigned int hi846_af_checksum_addr = 0;

    LOG_INF("%s enter\n",__func__);

    if (HI846_Sensor_OTP_read(0x0236) == 0x01) {
        hi846_af_addr = 0x0237;
        hi846_af_checksum_addr = 0x023c;
    } else if(HI846_Sensor_OTP_read(0x0236) == 0x13) {
        hi846_af_addr = 0x023d;
        hi846_af_checksum_addr = 0x0242;
    } else if(HI846_Sensor_OTP_read(0x0236) == 0x37) {
        hi846_af_addr = 0x0243;
        hi846_af_checksum_addr = 0x0248;
    } else
        return 0;
    
    LOG_INF("%s,hi846_af_addr = 0x%x\n",__func__,hi846_af_addr);

    for (loop = 0; loop < HI846_AF_DATA_SIZE; loop++){
			tt = HI846_Sensor_OTP_read(hi846_af_addr);
			Hi846_af_data[loop] = tt;
			hi846_af_addr++;
    }
    for(loop = 0; loop < HI846_AF_DATA_SIZE; loop++)
    {
			pr_err("=== AAA AF[%d] : %x AAA ===\n", loop, Hi846_af_data[loop]);
			checksum += Hi846_af_data[loop];
    }
		checksum_value = HI846_Sensor_OTP_read(hi846_af_checksum_addr);

    LOG_INF("%s: Hi846_af checksum_value = 0x%x checksum = 0x%x,0x%x\n",__func__, checksum_value, checksum,((checksum % 0xff) + 1));
    if(((checksum % 0xff) + 1) == checksum_value)
			LOG_INF("%s: af data checksum PASS\n",__func__);
    else{
			LOG_INF("%s: af data checksum Fail\n",__func__);
			return 0;
    }
    LOG_INF("%s exit\n",__func__);
    return 1;
...
kal_uint16 hi846_get_module_id(void)
{
    kal_uint16 ret = 0;
    kal_uint16 ModuleVendorID = 0;
    HI846OTPSetting();

    ModuleVendorID = Hi846_Sensor_OTP_info();
    if(ModuleVendorID == 0)
        return ModuleVendorID;// checksum fail return error

    ret = Hi846_Sensor_OTP_read_lsc();
    if(ret == 0)
        return ret;// checksum fail return error

    ret = HI846_Sensor_OTP_read_awb();
    if(ret == 0)
        return ret;// checksum fail return error

    ret = HI846_Sensor_OTP_read_af();
    if(ret == 0)
        return ret;// checksum fail return error
...
#if Hi846_OTP_FUNCTION
				sensor_init();
				hi846_module_id = hi846_get_module_id();
				LOG_INF("i2c write id  : 0x%x, sensor id: 0x%x,hi846_module_id: 0x%x\n",              
                  imgsensor.i2c_write_id,*sensor_id,hi846_module_id);
				if(0x19 != hi846_module_id){// if module ID is not correct, Must set *sensor_id to 0xFFFFFFFF for exit
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if (enable) { 
		...
		write_cmos_sensor(0x0A00, 0x0000);
		write_cmos_sensor(0x0a04, 0x0140);//ADPC OFF For Test Pattern
