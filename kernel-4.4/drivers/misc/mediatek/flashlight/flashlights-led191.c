...
#define FLASH_NODE//bug430198, guoyang.wt, ADD, 20190222, fix factory led can not turn on
...
int set_duty(struct pinctrl_state *led191_state_low, struct pinctrl_state *led191_state_high){
        int i = 0, ret;
	if(g_flash_duty > 15)
		g_flash_duty = 15;
	/* First rise for t_Ready high_level should > 150us */
	ret = pinctrl_select_state(led191_pinctrl, led191_state_high);
        udelay(155);
        ret = pinctrl_select_state(led191_pinctrl, led191_state_low);
	udelay(1);
        for(i = 0; i < g_flash_duty; i++){
		ret = pinctrl_select_state(led191_pinctrl, led191_state_high);
		udelay(3);
		if(i < g_flash_duty -1){
			ret = pinctrl_select_state(led191_pinctrl, led191_state_low);
...
static int led191_pinctrl_set(int pin, int state)
{
	...
	case LED191_PINCTRL_PIN_HWEN:
		if (state == LED191_PINCTRL_PINSTATE_LOW && !IS_ERR(led191_hw_chx_low))
			...
			if(0 != g_flash_duty)
			      ret = set_duty(led191_hw_chx_low, led191_hw_chx_high);
			else
...
static int led191_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	if(0 != g_flash_duty)//for flash mode
		g_flash_channel_idx = 0;
	else if(g_flash_duty == 0)//for preflash and torch using torch mode
...
//+bug430198, guoyang.wt, ADD, 20190222, fix factory led can not turn on
#ifdef FLASH_NODE
...
static ssize_t led_flash_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size){
  ...
  switch(value){
    ...
    break;
    case 1:
	    g_flash_duty = 0;    //LGE_CHANGE, 2019-04-16, Factory mode flash abnormal by wingtech, soonyong.kwon@lge.com
	    g_flash_channel_idx = 1;
