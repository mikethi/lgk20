...
void lge_convert_ntcode_format_to_string (struct FactoryNetworkCode *mtk_atcmd_ntcode, char *mcc, char *mnc, char *subset)
{
	...
	strncpy(out3, buffer, 3);

	strncpy (g_lge_mcc, out1, 3);
	strncpy (g_lge_mnc, out2, 3);
	strncpy (g_lge_subset, out3, 3);
}
#endif
	...
	for (i = BOOT_INFO; i < FEATURE_COUNT; i++) {
		...
		if (rt_feature.support_info.support_mask == CCCI_FEATURE_MUST_SUPPORT) {
			...
			case MISC_INFO_NTCODE:
				...
				lge_convert_ntcode_format_to_string (temp_buf_ntcode, mcc, mnc, subset); //NTCODE
				memcpy((char*)&(rt_f_element.feature[0]), mcc, 4);
				memcpy((char*)&(rt_f_element.feature[1]), mnc, 4);
				memcpy((char*)&(rt_f_element.feature[2]), subset, 4);
			...
			case MISC_INFO_2NTCODE:
				...
				lge_convert_ntcode_format_to_string (&temp_buf_ntcode[1], mcc2, mnc2, subset2); //2nd NTCODE
				memcpy((char*)&(rt_f_element.feature[0]), mcc2, 4);
				memcpy((char*)&(rt_f_element.feature[1]), mnc2, 4);
				memcpy((char*)&(rt_f_element.feature[2]), subset2, 4);
