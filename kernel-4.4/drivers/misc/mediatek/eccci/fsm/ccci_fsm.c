...
static void get_ntcode_data() {
	bool result = false;
	unsigned short networkCodeListNum = 2;
  ...
  if (temp_buf_ntcode == NULL)
		temp_buf_ntcode = (struct FactoryNetworkCode *)kmalloc(EMMC_BLOCK_SIZE, GFP_KERNEL);
	if (temp_buf_ntcode == NULL)
	{
	...
	{
		memset(temp_buf_ntcode, 0x00, EMMC_BLOCK_SIZE);
		result = LGE_FacReadNetworkCode_SBP (temp_buf_ntcode, networkCodeListNum, LGE_FAC_NETWORK_CODE_SIZE_SBP);
		if (result == true)
		{
