...
/*----------------------------------------------------------------------------*/
#define KXTJ2_1009_AXIS_X          1   //add by renqiang.wt for set direction 20190410
#define KXTJ2_1009_AXIS_Y          0   //add by renqiang.wt for set direction 20190410
...
#define KXTJ2_CAL_FAIL 1
#define KXTJ2_FLAT_FAIL 2

#define KXTJ2_AXIS_OLD_X       3
#define KXTJ2_AXIS_OLD_Y       4
#define KXTJ2_AXIS_OLD_Z       5
#define KXTJ2_DEFECT_LIMIT_XY 183
#define KXTJ2_DEFECT_LIMIT_Z1 760
#define KXTJ2_DEFECT_LIMIT_Z2 1288
#define KXTJ2_SHAKING_DETECT_THRESHOLD 144
#define KXTJ2_FAST_CALIB_TILT_LIMIT 187
...
struct kxtj2_1009_i2c_data {
    ...
    int                     cali_sw[KXTJ2_1009_AXES_NUM+5];
...
static const struct of_device_id accel_of_match[] = {
	...
	{.compatible = "mediatek,gsensor_1"},    //add by renqiang.wt for g-sensor 20190409
	{},
};
#endif
...
/*Kionix Auto-Cali Start*/
#ifdef KIONIX_AUTO_CAL
        raw[0]=-obj->data[KXTJ2_1009_AXIS_X]; //add by renqiang.wt for set direction 20190410
...
/*----------------------------------------------------------------------------*/
static int kxtj2_1009_do_calibration(struct i2c_client *client)
{
    int err = 0;
    s16 data[KXTJ2_1009_AXES_NUM] = {0,};
    s32 sum[KXTJ2_1009_AXES_NUM] = {0,};
    int cali[6] = {0,};
    int i = 0;
    unsigned int timeout_shaking = 0;
    unsigned int tilt_check_count = 0;
    struct kxtj2_1009_i2c_data *client_data = obj_i2c_data;
    struct kxtj2_1009_axis_data_t kxtj2_1009_udata_pre = {0,0,0};
    struct kxtj2_1009_axis_data_t kxtj2_1009_udata = {0,0,0};
    ...
    err = KXTJ2_1009_ReadData(client, data);
    if(err) {
        GSE_ERR("Fail read read data\n");
        return err;
    }

    kxtj2_1009_udata_pre.x = data[KXTJ2_1009_AXIS_X];
    kxtj2_1009_udata_pre.y = data[KXTJ2_1009_AXIS_Y];
    kxtj2_1009_udata_pre.z = data[KXTJ2_1009_AXIS_Z];
    ...
    do{
        ...
        kxtj2_1009_udata.x = data[KXTJ2_1009_AXIS_X];
        kxtj2_1009_udata.y = data[KXTJ2_1009_AXIS_Y];
        kxtj2_1009_udata.z = data[KXTJ2_1009_AXIS_Z];

        GSE_LOG("===========moved x, y, z============== timeout = %d\n", timeout_shaking);
        GSE_LOG("(%d, %d, %d) (%d, %d, %d)\n",
                kxtj2_1009_udata_pre.x,kxtj2_1009_udata_pre.y,kxtj2_1009_udata_pre.z,
                kxtj2_1009_udata.x,kxtj2_1009_udata.y, kxtj2_1009_udata.z);

        if((abs(kxtj2_1009_udata.x - kxtj2_1009_udata_pre.x) > KXTJ2_SHAKING_DETECT_THRESHOLD)
                ||(abs(kxtj2_1009_udata.y - kxtj2_1009_udata_pre.y) > KXTJ2_SHAKING_DETECT_THRESHOLD)
                ||(abs(kxtj2_1009_udata.z - kxtj2_1009_udata_pre.z) > KXTJ2_SHAKING_DETECT_THRESHOLD)){
            GSE_ERR("==============shaking x, y, z=============\n");
            return KXTJ2_CAL_FAIL;
        } else {
            kxtj2_1009_udata_pre.x = kxtj2_1009_udata.x;
            kxtj2_1009_udata_pre.y = kxtj2_1009_udata.y;
            kxtj2_1009_udata_pre.z = kxtj2_1009_udata.z;

            sum[KXTJ2_1009_AXIS_X] += kxtj2_1009_udata.x;
            sum[KXTJ2_1009_AXIS_Y] += kxtj2_1009_udata.y;
            sum[KXTJ2_1009_AXIS_Z] += kxtj2_1009_udata.z;

            if(abs(kxtj2_1009_udata.y)>KXTJ2_FAST_CALIB_TILT_LIMIT
                    ||abs(kxtj2_1009_udata.x)>KXTJ2_FAST_CALIB_TILT_LIMIT)
                tilt_check_count++;
            if(tilt_check_count > 5){
                GSE_ERR("========= tilted over 15 degree ==========\n");
                return KXTJ2_FLAT_FAIL;
            }
        }
    ...
    /* zero-g offset check */
    if(abs(sum[KXTJ2_1009_AXIS_X]/CALIBRATION_DATA_AMOUNT) > KXTJ2_DEFECT_LIMIT_XY ||
            abs(sum[KXTJ2_1009_AXIS_Y]/CALIBRATION_DATA_AMOUNT) > KXTJ2_DEFECT_LIMIT_XY ||
            ((abs(sum[KXTJ2_1009_AXIS_Z]/CALIBRATION_DATA_AMOUNT) < KXTJ2_DEFECT_LIMIT_Z1) ||
                (abs(sum[KXTJ2_1009_AXIS_Z]/CALIBRATION_DATA_AMOUNT) > KXTJ2_DEFECT_LIMIT_Z2))){
        GSE_ERR("========= Calibration zero-g offset check failed (%d %d %d) ==========\n",
            sum[KXTJ2_1009_AXIS_X]/CALIBRATION_DATA_AMOUNT,
            sum[KXTJ2_1009_AXIS_Y]/CALIBRATION_DATA_AMOUNT,
            sum[KXTJ2_1009_AXIS_Z]/CALIBRATION_DATA_AMOUNT);

        return KXTJ2_FLAT_FAIL;
    }

    /*Set new calibration*/
    client_data->cali_sw[KXTJ2_1009_AXIS_X] = -(s16)(sum[KXTJ2_1009_AXIS_X]/CALIBRATION_DATA_AMOUNT);
    client_data->cali_sw[KXTJ2_1009_AXIS_Y] = -(s16)(sum[KXTJ2_1009_AXIS_Y]/CALIBRATION_DATA_AMOUNT);
    if(sum[KXTJ2_1009_AXIS_Z]/CALIBRATION_DATA_AMOUNT > 0)
        client_data->cali_sw[KXTJ2_1009_AXIS_Z] = -(s16)(sum[KXTJ2_1009_AXIS_Z]/CALIBRATION_DATA_AMOUNT) + 1024;
    else
        client_data->cali_sw[KXTJ2_1009_AXIS_Z] = -(s16)(sum[KXTJ2_1009_AXIS_Z]/CALIBRATION_DATA_AMOUNT) - 1024;

    client_data->cali_sw[KXTJ2_AXIS_OLD_X] = cali[KXTJ2_1009_AXIS_X];
    client_data->cali_sw[KXTJ2_AXIS_OLD_Y] = cali[KXTJ2_1009_AXIS_Y];
    client_data->cali_sw[KXTJ2_AXIS_OLD_Z] = cali[KXTJ2_1009_AXIS_Z];
...
static ssize_t show_run_fast_calibration_value(struct device_driver *ptDevDriver, char *pbbuf)
{
    struct kxtj2_1009_i2c_data   *obj = obj_i2c_data;
    return sprintf(pbbuf, "%d\n", atomic_read(&obj->calibration));
}
static ssize_t store_run_fast_calibration_value(struct device_driver *ptDevDrv, const char *pbbuf, size_t tCount)
{
    int res = 0;
    struct i2c_client *client = kxtj2_1009_i2c_client;
    struct kxtj2_1009_i2c_data *obj = obj_i2c_data;
    ...
    res = kxtj2_1009_do_calibration(client);
    if(res != KXTJ2_SUCCESS)
    {
        GSE_LOG("Calibration fail.\n");
        atomic_set(&obj->calibration, res);
    }
    else
    {
        GSE_LOG("Calibration success.\n");
        atomic_set(&obj->calibration, KXTJ2_SUCCESS);
    }

    if( false == sensor_power){
        res = KXTJ2_1009_SetPowerMode(client, false);
        if (res)
            GSE_ERR("Power off KXTJ2 error %d!\n", res);
    }
...
static DRIVER_ATTR(run_fast_calibration, S_IWUSR | S_IRUGO, show_run_fast_calibration_value,  store_run_fast_calibration_value);
...
static long kxtj2_1009_unlocked_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		#if 1	//  sizeof(struct GSENSOR_VECTOR3D) sizeof(GSENSOR_VECTOR3D)
...
{
	int ret = 0;	
	ret = acc_driver_add(&kxtj2_1009_init_info);
