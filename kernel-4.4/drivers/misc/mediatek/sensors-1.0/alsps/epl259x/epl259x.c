...
#define DRIVER_VERSION          "3.0.8_lsrc_ANDR_O"
...
u16 ps_factory_offset = 99;
u16 ps_factory_hystersis = 32;
u16 ps_self_test_max_ct = 0x1f4;
u32 ps_self_ps_avg;
#endif
...
#if MN_SELF_TEST
static int epl259x_cal_ps_calibration( struct epl_sensor_priv* obj, int calib_times, bool write)
{
    ...
    ps_time = ps_sensing_time(epl_sensor.ps.integration_time, epl_sensor.ps.osr, epl_sensor.ps.cycle);
    als_time = als_sensing_time(epl_sensor.als.integration_time, epl_sensor.als.osr, epl_sensor.als.cycle);
    wait_time = wait_value[epl_sensor.ps_wait >> 4];
    ...
    20190115 solve I2C conflict add mutex_lock */
    mutex_lock(&sensor_mutex);
    epl_sensor_I2C_Write(obj->client, DEVREG_RESET, (EPL_POWER_OFF | EPL_RESETN_RESET));
    epl_sensor_I2C_Write(obj->client, DEVREG_PS_INT, (epl_sensor.interrupt_control | epl_sensor.ps.persist | EPL_INTTY_DISABLE));
    ...
    for(i=0; i<calib_times; i++)
    {
        ...
        ps_ct[i] = epl_sensor.ps.data.data;
        APS_LOG("[epl_sensor_do_calibration] sleep %dms, pdata[%d] = %d\n", sleep_time, i, ps_ct[i]);
    ...
    mutex_lock(&sensor_mutex);
    epl_sensor_I2C_Write(obj->client, DEVREG_RESET, (EPL_POWER_OFF | EPL_RESETN_RESET));
    epl_sensor_I2C_Write(obj->client, DEVREG_PS_INT, (epl_sensor.interrupt_control | epl_sensor.ps.persist | epl_sensor.ps.interrupt_type));
    epl_sensor_I2C_Write(obj->client, DEVREG_RESET, (EPL_POWER_ON | EPL_RESETN_RUN));
    mutex_unlock(&sensor_mutex);
    /* disable ps interrupt end */


    APS_LOG("[epl_sensor_do_calibration]  ps_raw_min=%u, ps_raw_max=%u\r\n", ps_raw_min, ps_raw_max);
    APS_LOG("[epl_sensor_do_calibration]  ps_self_ps_avg=%u, ps_self_test_max_ct=%u\r\n", (u16)ps_self_ps_avg, (u16)ps_self_test_max_ct);
    ...
    if(ps_raw_total > ps_self_test_max_ct)
    {
        ...
        APS_LOG("[epl_sensor_do_calibration] Failed \r\n");
        if(pflag == 0)
        {
            pflag =1;
            enable_ps= false;
            epl_sensor_enable_ps(false);
        }
            return CALI_FAIL;
    }

    APS_LOG("[epl_sensor_do_calibration] success \r\n");
    if(write == true)
    {
        ps_factory_ct = ps_self_ps_avg;
        epl_sensor.ps.high_threshold = ps_factory_ct + ps_factory_offset;
        epl_sensor.ps.low_threshold = epl_sensor.ps.high_threshold - ps_factory_hystersis;
        set_psensor_intr_threshold(epl_sensor.ps.low_threshold, epl_sensor.ps.high_threshold);
    }
...
static int epl259x_do_ps_calibration( struct epl_sensor_priv* obj, int calib_times, bool write)
{
    ...
    for( i = 0; i < CALB_TIMES; i++){
        ret = epl259x_cal_ps_calibration(obj, calib_times, write);
        if(ret == CALI_SUCCESS)
            break;
    }
    if(i >= CALB_TIMES){
...
#if PS_11_BIT
	cal = obj->ps_crosstalk << 5;
    sensor_calibration_save(ID_PROXIMITY, &cal);
#else
...
static void initial_global_variable(struct i2c_client *client, struct epl_sensor_priv *obj)
{
    ...
    epl_sensor.wait = EPL_WAIT_50_MS;
    epl_sensor.ps_wait = EPL_WAIT_100_MS;
...
#else
    epl_sensor.als.report_type = CMC_BIT_RAW; //CMC_BIT_PRE_COUNT;
#endif
    ...
    epl_sensor.ps.integration_time = EPL_PS_INTT_520;
    epl_sensor.ps.gain = EPL_GAIN_MID;
    epl_sensor.ps.osr = EPL_PSALS_OSR_11;
    epl_sensor.ps.cycle = EPL_CYCLE_2;
...
#if PS_11_BIT
    epl_sensor.ps.data.ir_data = epl_sensor.ps.data.ir_data >> 5;
    epl_sensor.ps.data.data = epl_sensor.ps.data.data >> 5;
#endif
...
void epl_sensor_update_mode(struct i2c_client *client)
{
    ...
    switch((enable_als << 1) | enable_ps)
    {
        case 0: //disable all
            epl_sensor.mode = EPL_MODE_IDLE;
			wait_time = epl_sensor.wait;
            break;
        case 1: //als = 0, ps = 1
            epl_sensor.mode = EPL_MODE_PS;
			wait_time = epl_sensor.ps_wait;
         break;
        case 2: //als = 1, ps = 0
    ...
    if(epl_sensor.revno == 0x0288)
        ...
        epl_sensor_I2C_Write(obj->client, DEVREG_ENABLE, wait_time | epl_sensor.mode);
...
static ssize_t epl_sensor_store_lsrc_scale(struct device_driver *ddri, const char *buf, size_t count)
{
    int scale=0;
...
#if MN_SELF_TEST
static ssize_t epl259x_show_ps_high_thd(struct device_driver *ddri, char *buf)
{
    int ps_hthd;
    ps_hthd = (int)epl_sensor.ps.high_threshold;
    return sprintf( buf, "%d\n", ps_hthd);
}
static ssize_t epl259x_store_ps_high_thd( struct device_driver *dev, const char *buf, size_t count )
{
    int ps_hthd=0;

    sscanf(buf, "%d", &ps_hthd);
    epl_sensor.ps.high_threshold = (u16)ps_hthd;
    set_psensor_intr_threshold(epl_sensor.ps.low_threshold, epl_sensor.ps.high_threshold);
    return count;
}

static ssize_t epl259x_show_ps_low_thd(struct device_driver *ddri, char *buf)
{
    int ps_lthd;
    ps_lthd = (int)epl_sensor.ps.low_threshold;
    return sprintf( buf, "%d\n", ps_lthd);
}
static ssize_t epl259x_store_ps_low_thd( struct device_driver *dev, const char *buf, size_t count )
{
    int ps_lthd=0;

    sscanf(buf, "%d", &ps_lthd);
    epl_sensor.ps.low_threshold = (u16)ps_lthd;
    set_psensor_intr_threshold(epl_sensor.ps.low_threshold, epl_sensor.ps.high_threshold);
    return count;
}

static ssize_t epl259x_show_ps_offset(struct device_driver *ddri, char *buf)
{
    return sprintf( buf, "%d\n", ps_factory_offset);
}
static ssize_t epl259x_store_ps_offset( struct device_driver *dev, const char *buf, size_t count )
{
    struct epl_sensor_priv *obj = epl_sensor_obj;
    int offset=0;
	...
	epl_sensor.ps.high_threshold = obj->ps_crosstalk + ps_factory_offset;
    epl_sensor.ps.low_threshold = epl_sensor.ps.high_threshold - ps_factory_hystersis;
...
static ssize_t epl259x_show_ps_hystersis(struct device_driver *ddri, char *buf)
{
    int hystersis;
    hystersis = (int)ps_factory_hystersis;
    return sprintf( buf, "%d\n", hystersis);
}
static ssize_t epl259x_store_ps_hystersis( struct device_driver *dev, const char *buf, size_t count )
{
    struct epl_sensor_priv *obj = epl_sensor_obj;
    int hystersis=0;
...
static ssize_t epl259x_show_cali_value(struct device_driver *ddri, char *buf)
{
...
static ssize_t epl259x_store_cali_value( struct device_driver *dev, const char *buf, size_t count )
{
    ...
    APS_LOG("epl259x_store_cali_value calib_times = %d \r\n",calib_times);

    if (calib_times < 10) calib_times = 10;

    obj->cali_status = CALI_FAIL;

    epl259x_do_ps_calibration(obj, calib_times, true);

    return count;
}

static ssize_t epl259x_show_ps_crosstalk ( struct device_driver *dev, char *buf )
{
    struct epl_sensor_priv *obj = epl_sensor_obj;
...
static ssize_t epl259x_store_ps_crosstalk ( struct device_driver *dev, const char *buf, size_t count )
{
...
static DRIVER_ATTR(als_lsrc_scale,			    S_IWUSR | S_IRUGO, NULL,                               epl_sensor_store_lsrc_scale                 );
#endif
#if MN_SELF_TEST
static DRIVER_ATTR (near_offset,                S_IWUSR | S_IRUGO, epl259x_show_ps_high_thd,          epl259x_store_ps_high_thd                 );
static DRIVER_ATTR (far_offset,                 S_IWUSR | S_IRUGO, epl259x_show_ps_low_thd,           epl259x_store_ps_low_thd                 );
static DRIVER_ATTR (target_pdata,                S_IWUSR | S_IRUGO, epl259x_show_ps_offset,            epl259x_store_ps_offset                 );
static DRIVER_ATTR (near_to_far,               S_IWUSR | S_IRUGO, epl259x_show_ps_hystersis,            epl259x_store_ps_hystersis                 );
static DRIVER_ATTR (cali,                       S_IWUSR | S_IRUGO, epl259x_show_cali_value,           epl259x_store_cali_value                 );
static DRIVER_ATTR (ps_crosstalk,               S_IWUSR | S_IRUGO, epl259x_show_ps_crosstalk,         epl259x_store_ps_crosstalk               );
#endif
/*----------------------------------------------------------------------------*/
