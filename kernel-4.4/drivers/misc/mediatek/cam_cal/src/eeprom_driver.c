...
extern unsigned char hi846_lsc_data[LSC_DATA_SIZE];
#define AWB_DATA_SIZE 8
extern unsigned char Hi846_awb_data[AWB_DATA_SIZE];
#define AF_DATA_SIZE 5
extern unsigned char Hi846_af_data[AF_DATA_SIZE];
#define VENDOR_ID_DATA_SIZE 1
extern unsigned char Hi846_vendor_id;
#endif
...
#define GC8034_OTP_FUNCTION 1
#if GC8034_OTP_FUNCTION
#define GC8034_OTP_SIZE 0x771
#define GC8034_AWB_DATA_START 0x0A
#define GC8034_AF_DATA_START 0x1D
#define GC8034_LSC_DATA_START 0x23
#define GC8034_LSC_MID_START 1
#define GC8034_AWB_DATA_SIZE 16
#define GC8034_AF_DATA_SIZE 4
#define GC8034_LSC_DATA_SIZE 1868
extern unsigned char gc8034_otp_data[GC8034_OTP_SIZE];
extern unsigned char gc8034_module_info_is_valid;
extern unsigned char gc8034_awb_is_valid;
extern unsigned char gc8034_lsc_is_valid;
extern unsigned char gc8034_af_is_valid;
#endif

#define GC5035_OTP_FUNCTION 1
#if GC5035_OTP_FUNCTION
#define GC5035_AWB_DATA_SIZE 4
#define GC5035_AWB_INFO_SIZE 5
extern unsigned char gc5035_otp_data_awb[GC5035_AWB_INFO_SIZE];
...
#endif
{
	...
	case CAM_CALIOC_G_READ:
		PK_DBG("CAM_CALIOC_G_READ start! offset=%d, length=%d\n", ptempbuf->u4Offset,
			  ptempbuf->u4Length);
#if Hi846_OTP_FUNCTION
		if (ptempbuf->sensorID == 0x846 && ptempbuf->u4Length == 0x1 && ptempbuf->u4Offset == 0x202){//HI846 MID data
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)&Hi846_vendor_id, ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
		...
		if (ptempbuf->sensorID == 0x846 && ptempbuf->u4Length == 0x74C && ptempbuf->u4Offset == 0x24a){//HI846 LSC data
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &hi846_lsc_data[0], ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
				pr_debug("ioctl copy to user failed\n");
				return -EFAULT;
		...
		if (ptempbuf->sensorID == 0x846 && ptempbuf->u4Length == 0x08  && ptempbuf->u4Offset == 0x021b){//HI846 AWB data
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &Hi846_awb_data[0], ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
				pr_debug("ioctl copy to user failed\n");
				return -EFAULT;
		...
		if (ptempbuf->sensorID == 0x846 && ptempbuf->u4Length == 0x05  && ptempbuf->u4Offset == 0x0237){//HI846 AF data
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &Hi846_af_data[0], ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
				pr_debug("ioctl copy to user failed\n");
				return -EFAULT;
...
#endif
#if HI556_OTP_FUNCTION
		if (ptempbuf->sensorID == 0x556 && ptempbuf->u4Length == 0x1 && ptempbuf->u4Offset == 0x402){//HI556 MID data
		...
		if (ptempbuf->sensorID == 0x556 && ptempbuf->u4Length == 0x74C && ptempbuf->u4Offset == 0x453 && hi556_lsc_valid){//HI556 LSC data
		...
		if (ptempbuf->sensorID == 0x556 && ptempbuf->u4Length == 0x10 && ptempbuf->u4Offset == 0x41d && hi556_awb_valid){//HI556 AWB data
...
#endif
#if GC8034_OTP_FUNCTION
		if (ptempbuf->sensorID == 0x8044 && ptempbuf->u4Length == 0x1 && ptempbuf->u4Offset == GC8034_LSC_MID_START && gc8034_module_info_is_valid){//GC8034 MID data
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)&gc8034_otp_data[GC8034_LSC_MID_START], ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
				pr_debug("ioctl copy to user failed\n");
				return -EFAULT;
		  }
			i4RetValue = GC8034_LSC_MID_START;
			kfree(pBuff);
			kfree(pu1Params);
			return i4RetValue;
		}
		if (ptempbuf->sensorID == 0x8044 && ptempbuf->u4Length == 0x74C && ptempbuf->u4Offset == GC8034_LSC_DATA_START && gc8034_lsc_is_valid){//GC8034 LSC data
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &gc8034_otp_data[GC8034_LSC_DATA_START], ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
				pr_debug("ioctl copy to user failed\n");
				return -EFAULT;
		  }
			i4RetValue = GC8034_LSC_DATA_SIZE;
			kfree(pBuff);
			kfree(pu1Params);
			return i4RetValue;
		}
		if (ptempbuf->sensorID == 0x8044 && ptempbuf->u4Length == GC8034_AWB_DATA_SIZE && ptempbuf->u4Offset == GC8034_AWB_DATA_START && gc8034_awb_is_valid){//GC8034 AWB data
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &gc8034_otp_data[GC8034_AWB_DATA_START], ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
				pr_debug("ioctl copy to user failed\n");
				return -EFAULT;
		  }
			i4RetValue = GC8034_AWB_DATA_SIZE;
			kfree(pBuff);
			kfree(pu1Params);
			return i4RetValue;
		}
		if (ptempbuf->sensorID == 0x8044 && ptempbuf->u4Length == GC8034_AF_DATA_SIZE && ptempbuf->u4Offset == GC8034_AF_DATA_START && gc8034_af_is_valid){//GC8034 AF data
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &gc8034_otp_data[GC8034_AF_DATA_START], ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
				pr_debug("ioctl copy to user failed\n");
				return -EFAULT;
		  }
			i4RetValue = GC8034_AF_DATA_SIZE;
			kfree(pBuff);
			kfree(pu1Params);
			return i4RetValue;
		}
#endif
#if GC5035_OTP_FUNCTION
		if (ptempbuf->sensorID == 0x5035 && ptempbuf->u4Length == 0x1 && ptempbuf->u4Offset == 0x018 && gc5035_module_id){//GC5035 MID data
		...
		if (ptempbuf->sensorID == 0x5035 && ptempbuf->u4Length == 0x04 && ptempbuf->u4Offset == 0x080 && gc5035_awb_valid){//GC5035 AWB data
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &gc5035_otp_data_awb[0], ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
				pr_debug("ioctl copy to user failed\n");
				return -EFAULT;
		  }
			i4RetValue = GC5035_AWB_DATA_SIZE;
			kfree(pBuff);
			kfree(pu1Params);
			return i4RetValue;
		}
