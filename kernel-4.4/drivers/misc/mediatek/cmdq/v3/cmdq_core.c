	...
	const char *tag)
{
	...
	list_for_each_entry(pResource, &gCmdqContext.resourceList, list_entry) {
		if (resourceEvent == pResource->lockEvent) {
			...
			CMDQ_LOG("[%s][Res]   call release: %llu, done release: %lld\n",
				tag, pResource->call_release, pResource->done_release);
		...
		if (engineFlag & pResource->engine_flag) {
			...
			if (!pResource->used) {
				...
				if (pResource->releaseCB == NULL) {
					...
					pResource->call_release = sched_clock();
					status = cb_func(pResource->lockEvent);
					pResource->done_release = sched_clock();
	...
	u64 *engine_flag_out)
{
	...
	CMDQ_LOG("[Res] Release resource with event: %d\n", resourceEvent);
	list_for_each_entry(pResource, &gCmdqContext.resourceList, list_entry) {
		if (resourceEvent == pResource->lockEvent) {
			mutex_lock(&gCmdqResourceMutex);
			/* find matched resource */
