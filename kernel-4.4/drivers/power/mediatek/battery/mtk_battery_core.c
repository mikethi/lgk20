...
#if !defined(MTK_GAUGE_PARAM_TO_DTS)
...
#else /*!defined(MTK_GAUGE_PARAM_TO_DTS)*/
void fg_custom_init_from_header(void)
{
	fgauge_get_profile_id();
...
#endif /*!defined(MTK_GAUGE_PARAM_TO_DTS)*/
...
void fg_custom_init_from_dts(struct platform_device *dev)
{
	struct device_node *np = dev->dev.of_node;
	unsigned int val;
	int bat_id, multi_battery, active_table, i, j;
	char node_name[128];
	...
	fg_cust_data.com_fg_meter_resistance = fg_cust_data.fg_meter_resistance;
	fg_read_dts_val(np, "COM_FG_METER_RESISTANCE",
		&(fg_cust_data.com_fg_meter_resistance), 1);

	fg_read_dts_val(np, "R_FG_VALUE", &(fg_cust_data.r_fg_value),
		UNIT_TRANS_10);
	fg_cust_data.com_r_fg_value = fg_cust_data.r_fg_value;
	fg_read_dts_val(np, "COM_R_FG_VALUE", &(fg_cust_data.com_r_fg_value),
		UNIT_TRANS_10);
	...
	fg_read_dts_val(np, "RECORD_LOG",
		&(fg_cust_data.record_log), 1);
	fg_read_dts_val(np, "DIFFERENCE_FULLOCV_ITH",
		&(fg_cust_data.difference_fullocv_ith), UNIT_TRANS_10);
	fg_read_dts_val(np, "SHUTDOWN_1_TIME", &(fg_cust_data.shutdown_1_time),
		1);
	...
	fg_read_dts_val(np, "PMIC_SHUTDOWN_CURRENT",
		&(fg_cust_data.pmic_shutdown_current), 1);
	fg_read_dts_val(np, "CAR_TUNE_VALUE",
		&(fg_cust_data.car_tune_value), UNIT_TRANS_10);
	fg_read_dts_val(np, "MULTI_GAUGE0_EN",
		&(fg_cust_data.multi_temp_gauge0), 1);
	...
	for (i = 0; i < fg_table_cust_data.active_table_number; i++) {
		...
		fg_custom_parse_table(np, node_name,
			fg_table_cust_data.fg_profile[i].fg_profile);
	}
}
#endif /* !defined(MTK_GAUGE_PARAM_TO_DTS) */
#endif /* CONFIG_OF */
...
{
	bm_err("[%s] T0: %4d Q_MAX: %4d\n", __func__,
			fg_table_cust_data.fg_profile[0].temperature,
			fg_table_cust_data.fg_profile[0].q_max);
	bm_err("[%s] T1: %4d Q_MAX: %4d\n", __func__,
			fg_table_cust_data.fg_profile[1].temperature,
			fg_table_cust_data.fg_profile[1].q_max);
	bm_err("[%s] T2: %4d Q_MAX: %4d\n", __func__,
			fg_table_cust_data.fg_profile[2].temperature,
			fg_table_cust_data.fg_profile[2].q_max);
	bm_err("[%s] T3: %4d Q_MAX: %4d\n", __func__,
			fg_table_cust_data.fg_profile[3].temperature,
			fg_table_cust_data.fg_profile[3].q_max);
#ifdef CONFIG_MTK_ADDITIONAL_BATTERY_TABLE
	bm_err("[%s] T4: %4d Q_MAX: %4d\n", __func__,
			fg_table_cust_data.fg_profile[4].temperature,
			fg_table_cust_data.fg_profile[4].q_max);
#endif
}
#endif
...
void fg_daemon_comm_INT_data(char *rcv, char *ret)
{
	...
	bm_debug("%s type:%d in:%d out:%d,Retun t:%d,in:%d,o:%d,s:%d\n",
		...
		if (prcv->type == FG_SET_AGING_FACTOR
			&& battery_main.BAT_RAW_CAPACITY >= 95) {
