...
struct battery_data battery_main = {
	...
	.BAT_batt_vol = 0,
#ifdef CONFIG_LGE_PM
	.BAT_batt_temp = -127,		/* flag as not initialized */
#else /* MediaTek */
	.BAT_batt_temp = 0,
#endif
#ifdef CONFIG_LGE_PM
...
int force_get_tbat_internal(bool update, int *bat_temperature_precise)
{
	...
	if (is_battery_init_done() == false) {
		*bat_temperature_precise = 250;
		return 25;
	}

	if (gm.fixed_bat_tmp != 0xffff) {
		*bat_temperature_precise = gm.fixed_bat_tmp * 10;
		return gm.fixed_bat_tmp;
	}

	if (get_ec()->fixed_temp_en) {
		*bat_temperature_precise = get_ec()->fixed_temp_value * 10;
	...
	*bat_temperature_precise = bat_temperature_val;
...
int force_get_tbat(bool update)
{
	...
	bat_temperature_val = force_get_tbat_internal(update, &tbat_precise);
	...
	if (bat_temperature_val <= fg_cust_data.battery_tmp_to_disable_nafg) {
		...
		tbat_precise = DEFAULT_BATTERY_TMP_WHEN_DISABLE_NAFG * 10;
		bm_debug("[force_get_tbat] %d %d\n", bat_temperature_val,
			tbat_precise);
	...
	bm_debug("[force_get_tbat] %d %d\n", bat_temperature_val, tbat_precise);
