...
#ifdef CONFIG_LGE_PM
bool is_fastchg_supported(void)
{
	if (pinfo == NULL)
		return false;
...
int charger_manager_enable_hz(struct charger_consumer *consumer, bool en)
{
	...
	ret = charger_dev_is_input_suspended(chg_dev, &is_en);
	if (ret < 0) {
		chr_err("%s: get is input suspended failed\n", __func__);
		return ret;
	}

	if (is_en == info->enable_hz)
		return 0;

	pr_info("%s: user: %s, en = %d\n", __func__,
		dev_name(consumer->dev), info->enable_hz);

	return charger_dev_input_suspend(chg_dev, info->enable_hz);
}
...
#ifdef CONFIG_LGE_PM
			/* do not check chr_type for type-c */
#else /* MediaTek */
...
#ifdef CONFIG_LGE_PM
static int mtk_charger_plug_change(struct charger_manager *info,
				   enum charger_type chr_type)
{
	...
	info->safety_timeout = false;
	chr_err("mtk_is_charger_on plug change, type:%d\n", chr_type);
...
#ifdef CONFIG_LGE_PM
		else if (info->chr_event || info->chr_type != chr_type)
			mtk_charger_plug_change(info, chr_type);
#endif
	}

#ifdef CONFIG_LGE_PM
	info->chr_event = false;
#endif
...
#ifdef CONFIG_LGE_PM
	info->battery_voltage = battery_get_bat_voltage();
#endif
}
...
#ifdef CONFIG_LGE_PM
static int mtk_charger_en_ship_mode_show(struct seq_file *m, void *data)
{
	struct charger_manager *pinfo = m->private;
...
static ssize_t mtk_charger_en_ship_mode_write(struct file *file,
...
#ifdef CONFIG_LGE_PM
	proc_create_data("en_ship_mode", S_IRUGO | S_IWUSR, battery_dir,
			&mtk_charger_en_ship_mode_fops, info);
#endif
